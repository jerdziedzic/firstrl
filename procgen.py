...
def place_entities(
    ...
    for i in range(number_of_monsters):
        ...
        if not any(entity.x == x and entity.y ==y for entity in dungeon.entities): # Check if an entity already exists at x,y before placing one
            if random.random() < 0.8:
                entity_factories.orc.spawn(dungeon, x, y) # Place an Orc
            else:
                entity_factories.troll.spawn(dungeon, x, y) # Place a Troll
